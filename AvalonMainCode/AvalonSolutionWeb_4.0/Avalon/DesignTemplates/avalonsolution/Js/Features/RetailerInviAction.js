...
$("#RetailerWebsite").change(function () {
    var chkWebsite = $("#RetailerWebsite").val();
    if ($.trim(chkWebsite) != '') {
        //var emailReg = /^(http(s)?:\/\/(?:www\.|(?!www))[^\s\.]+\.[^\s]{2,}|www\.[^\s]+\.[^\s]{2,})$/;
        var emailReg = /^(http(s)?:\/\/()[^\s\.]+\.[^\s]{2,}|[^\s]+\.[^\s]{2,})$/;
        var emailaddressVal = $("#RetailerWebsite").val();
        if (!emailReg.test(emailaddressVal)) {
            $("#urlerrorRetailer").empty();
            $("#urlerrorRetailer").append("*Invalid Website URL");
            $("#RetailerWebsite").addClass('invalid');
            return false;
        }
        else {
            $("#urlerrorRetailer").empty();
            $("#RetailerWebsite").removeClass('invalid');
        }
    }
    else {
        $("#urlerrorRetailer").empty();
...
$("#SupplierPhone").change(function () {
    var SupplierPhone = $('#SupplierPhone').val();
    if ($.trim(SupplierPhone) != '') {
        $("#SupplierPhone").removeClass('invalid');
    }
});

$("#SupplierPhone").keydown(function (e) {

    var code = e.keyCode || e.which;
    if (code == '9') {
        var SupplierPhone = $("#SupplierPhone").val();
        if ($.trim(SupplierPhone) == '') {
            $("#SupplierPhone").addClass('invalid');
        }
        else {
            $("#SupplierPhone").removeClass('invalid');
        }
...
$("#SupplierContName").change(function () {
    var SupplierContName = $('#SupplierContName').val();
    if ($.trim(SupplierContName) != '') {
        $("#SupplierContName").removeClass('invalid');
    }
});

$("#SupplierContName").keydown(function (e) {

    var code = e.keyCode || e.which;
    if (code == '9') {
        var SupplierContName = $("#SupplierContName").val();
        if ($.trim(SupplierContName) == '') {
            $("#SupplierContName").addClass('invalid');
        }
        else {
            $("#SupplierContName").removeClass('invalid');
        }
...
$("#SupplierWebsite").change(function () {
    var chkWebsite = $("#SupplierWebsite").val();
    if ($.trim(chkWebsite) != '') {
        //var emailReg = /^(http(s)?:\/\/(?:www\.|(?!www))[^\s\.]+\.[^\s]{2,}|www\.[^\s]+\.[^\s]{2,})$/;
        var emailReg = /^(http(s)?:\/\/()[^\s\.]+\.[^\s]{2,}|[^\s]+\.[^\s]{2,})$/;
        ...
        if (!emailReg.test(emailaddressVal)) {
            ...
            $("#SupplierWebsite").addClass('invalid');
            return false;
        }
        else {
            $("#urlerror").empty();
            $("#SupplierWebsite").removeClass('invalid');
        }
    }
    else {
        $("#urlerror").empty();
   ...
   var RetailerEmail = $("#RetailerEmailID").val();
    ...
    if (isValid == true) {
       ...
       // jsondata.ReqEmailCC = $("#ReqEmailCC").val();
        ...
        //var url = sitesetting.AvalonDataServicesPath + "Features/ServicePages/RetailerInvitation.ashx";
        //alert(url);
        //$.getJSON(url, function (jsondata, json) {
        //    console.log(json);
        //    if (json == 'success') {
        //        Loader(false);
        ...
        $.ajax({
            method: 'GET',
            url: sitesetting.AvalonDataServicesPath + "Features/ServicePages/RetailerInvitation.ashx",
            headers: {
            ...
            success: function (data) {
                ...
                if (data == '"success"') {
                    ...
                    $('#confirmdiv').css({ display: "block" });
                    $('#invitationdiv').css({ display: "none" });
                    //$("#invitationdiv").trigger("reset");
                    $('#invitationdiv')[0].reset();
                }
                    //$('#confirmdiv').css({ display: "block" })
                    //$('#invitationdiv').css({ display: "none" })
